...
const OrderDetail = () => {
    ...
    const handleRequestReturn = async () => {
        const { value: formValues, isConfirmed } = await Swal.fire({
            ...
            html: `
                <div class="text-left space-y-4">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-gray-500 mt-1 italic">Khuyên dùng: Giúp Admin quét mã chuyển khoản chính xác và nhanh chóng hơn.</p>
                    </div>
                </div>
            `,
            showCancelButton: true,
            ...
            preConfirm: async () => {
                ...
                if (mediaInput.files.length === 0) { Swal.showValidationMessage('Vui lòng cung cấp ít nhất 1 hình ảnh/video chứng minh!'); return false; }
    ...
    const handleSubmitReturnTracking = async (approvedDate) => {
            ...
            html: `
                <div class="text-left space-y-5">
                    ...
                    <div class="bg-amber-50 border border-amber-200 p-3 rounded-lg text-xs text-amber-800 font-medium text-center">
                        <i class="fas fa-clock mr-1"></i> Gửi hàng và nhập mã vận đơn trước <strong class="text-red-600">${deadlineStr}</strong>.
                    </div>
                    ...
                    <div class="space-y-3 bg-gray-50 p-4 rounded-lg border border-gray-200">
                        ...
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-1">Mã vận đơn (Tracking Code) <span class="text-red-500">*</span></label>
                        ...
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-1">Ảnh chụp biên nhận gửi hàng <span class="text-gray-400 font-normal">(Tùy chọn)</span></label>
                            <input type="file" id="swal-ret-receipt" accept="image/*" class="w-full text-sm text-gray-600 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-white file:border-gray-300 file:border file:text-stone-700 hover:file:bg-gray-50">
                        </div>
                    </div>
                </div>
            `,
        ...
        if (isConfirmed) {
            ...
            try {
                ...
                Swal.fire('Thành công', 'Đã lưu thông tin vận đơn. Shop sẽ kiểm tra khi nhận được hàng.', 'success');
                loadOrder();
    ...
    // ✅ ĐÃ SỬA: Tính subTotal chuẩn xác theo giá sau khuyến mãi
                ...
                ${order.status === 'cancelled' || order.status === 'failed_delivery' ? 'bg-red-400' : 'bg-black'}`}
                        ...
                        {steps.map((step, index) => {
                            ...
                            return (
                                <div key={step.id} className="relative z-10 flex flex-col items-center w-16 md:w-24">
                                    <div className={`w-10 h-10 border-2 flex items-center justify-center transition-all duration-300 bg-white ${isError ? 'border-red-500 text-red-500' : isCompleted ? 'border-black text-black' : 'border-gray-200 text-stone-300'}`}>
                                        <FontAwesomeIcon icon={stepIcons[step.id]} className="text-sm" />
                                    </div>
                                    <p className={`mt-3 text-[10px] md:text-xs font-bold text-center leading-tight ${isActive ? (isError ? 'text-red-500' : 'text-black') : isCompleted ? 'text-stone-600' : 'text-stone-300'}`}>
                                        {step.label}
                                    </p>
                    ...
                    {isRejected && (
                        <div className="mt-6 bg-rose-50 border-l-4 border-rose-500 px-5 py-4">
                            <p className="text-[10px] uppercase tracking-widest font-bold text-rose-600 mb-1 flex items-center gap-1.5"><FontAwesomeIcon icon={['fas', 'triangle-exclamation']} /> Khiếu nại bị từ chối</p>
                        ...
                        {order.paymentMethod === 'transfer' && order.paymentStatus === 'Chờ thanh toán' && order.status !== 'cancelled' && (
                            <div className="bg-white border border-gray-100 p-6 flex flex-col md:flex-row gap-6 items-start">
                                ...
                                <div className="w-40 h-40 border border-gray-100 flex-shrink-0 overflow-hidden">
                                    <img src={`https://img.vietqr.io/image/vcb-1026325913-compact2.png?amount=${order.totalPrice}&addInfo=${order._id.slice(-6).toUpperCase()}&accountName=NGUYEN VAN A`} alt="QR Pay" className="w-full h-full object-cover" />
                                </div>
                            </div>
                        )}
                        ...
                        <div className="bg-white border border-gray-100 p-6">
                            ...
                            <div className="flex flex-col divide-y divide-gray-50">
                                {order.items.map((item) => {
                                    // ✅ ĐÃ SỬA: Lấy giá khuyến mãi hiển thị
                                    const currentPrice = item.book.discountedPrice || item.book.price;
                                    return (
                        ...
                        <div className="bg-white border border-gray-100 p-6">
                            ...
                            <div className="space-y-4 text-sm">
                                {[
                                    ...
                                    <div key={label}><span className="text-[9px] uppercase tracking-[0.2em] font-bold text-stone-400 block mb-0.5">{label}</span><span className="font-semibold text-black leading-relaxed">{value}</span></div>
                                ))}
                                <div>
                                    <span className="text-[9px] uppercase tracking-[0.2em] font-bold text-stone-400 block mb-1">Thanh toán</span>

          ...
          <div className="grid grid-cols-3 gap-4 mb-4">
            <div>
              ...
              <div className="relative">
                <input required value={form.importPrice !== '' ? Number(form.importPrice).toLocaleString('vi-VN') : ''} onChange={e => handleCurrencyChange('importPrice', e.target.value)} onKeyDown={e => handleCurrencyKeyDown(e, 'importPrice')} placeholder="0" className={`${inputCls} pr-7 font-mono font-semibold`} />
            ...
            <div>
              ...
              <div className="relative">
                <input required value={form.price !== '' ? Number(form.price).toLocaleString('vi-VN') : ''} onChange={e => handleCurrencyChange('price', e.target.value)} onKeyDown={e => handleCurrencyKeyDown(e, 'price')} placeholder="0" className={`${inputCls} pr-7 font-mono font-semibold`} />
                <span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-gray-400 font-bold pointer-events-none">₫</span>
              </div>
            </div>
            <div>
            ...
            <label className={`flex flex-col items-center justify-center w-full border-2 border-dashed rounded-xl cursor-pointer transition-all duration-200 ${dragging ? 'border-indigo-400 bg-indigo-50' : 'border-gray-200 bg-gray-50 hover:border-indigo-300 hover:bg-indigo-50/50'}`} onDragOver={e => { e.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={handleDrop}>
              ...
              {imagePreview ? (
                ...
                <div className="py-10 flex flex-col items-center gap-2">
                  <svg className="w-10 h-10 text-indigo-300" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4" /><polyline points="17 8 12 3 7 8" /><line x1="12" y1="3" x2="12" y2="15" /></svg>

...
/* ─── Style tokens ─────────────────────────────────── */
const FONT_MONO = { fontFamily: "'Space Mono', monospace" };
...
const Navbar = () => {
    ...
    const goToGenre = (g) => {
        navigate(`/books/view-all?genre=${encodeURIComponent(g)}`);
        setShowGenres(false);
        setMenuOpen(false);
    };
    ...
    useEffect(() => {
        if (document.getElementById('tomo-fonts')) return;
        const link = document.createElement('link');
        link.id = 'tomo-fonts';
        link.rel = 'stylesheet';
        link.href = 'https://fonts.googleapis.com/css2?family=Cormorant+Garamond:wght@300;400;500&family=Space+Mono:wght@400;700&display=swap';
        document.head.appendChild(link);
    }, []);

    return (
        <>
            <header className="sticky top-0 z-40 bg-white/95 backdrop-blur-sm" style={FONT_MONO}>
                {/* ════════════════════════════════════════
                    1 TIER DUY NHẤT - KHÔNG BORDER
                ════════════════════════════════════════ */}
                <div className="w-full px-6 lg:px-10 flex items-center justify-between h-16">
                        ...
                        <div className="relative flex items-center h-10" onMouseEnter={onGenreEnter} onMouseLeave={onGenreLeave}>
                            ...
                            <div className={`absolute left-1/2 -translate-x-1/2 top-full pt-2 z-50 transition-all duration-300 origin-top ${showGenres ? 'opacity-100 scale-y-100 translate-y-0' : 'opacity-0 scale-y-95 -translate-y-2 pointer-events-none'}`}>
                                <div className="w-56 bg-white py-3 shadow-2xl border border-gray-100">
                                    {GENRES.map((genre) => (
                                        <button key={genre} onClick={() => goToGenre(genre)} className="w-full text-left px-5 py-3 text-[11px] font-bold tracking-[0.2em] uppercase text-stone-500 hover:text-black hover:bg-stone-50 transition-colors">
                                            {genre}
                                        </button>
                                    ))}
                                </div>
                    ...
                    <div className="flex items-center gap-1 lg:gap-3">
                        {/* Cụm Icons (Tất cả màn hình) */}
                        <div className="flex items-center gap-1">
                            <InputSearch />
                            <NotificationBell />
                            <Link to="/favorites" className={iconBtn} title="Yêu thích">
                                <FontAwesomeIcon icon={['far', 'heart']} className="text-[18px]" />
                            </Link>
                            <Link to="/cart" className={iconBtn} title="Giỏ hàng">
                                <FontAwesomeIcon icon={['fas', 'bag-shopping']} className="text-[17px]" />
                                {cartCount > 0 && <span className="absolute top-1.5 right-1 inline-flex items-center justify-center min-w-[14px] h-[14px] px-1 text-[9px] font-bold text-white bg-black rounded-full">{cartCount}</span>}
                            </Link>
                        </div>

                        <div className="hidden lg:block w-px h-4 bg-gray-200 mx-2" />

                        {/* Cụm Auth (Chỉ hiện trên Desktop) */}
                        <div className="hidden lg:flex items-center">
                            {user ? (
                                <div className="relative" ref={dropdownRef}>
                                    <button onClick={() => setDropdownOpen(v => !v)} className="flex items-center gap-2.5 outline-none group">
                                        <div className="text-right leading-none">
                                            <p className="text-[11px] font-bold tracking-widest uppercase text-black group-hover:text-stone-500 transition-colors">{user.name || user.username}</p>
                                        </div>
                                        <img src={getAvatarUrl()} alt="Avatar" className="w-8 h-8 rounded-full object-cover transition-opacity group-hover:opacity-80" onError={(e) => { e.target.src = 'http://localhost:5000/uploads/avatars/default-user.png'; }} />
                                    </button>
                                    ...
                                    <div className={`absolute right-0 top-full mt-4 w-56 bg-white z-[999] transition-all duration-300 origin-top shadow-2xl border border-gray-100 ${dropdownOpen ? 'opacity-100 scale-100 translate-y-0' : 'opacity-0 scale-95 -translate-y-2 pointer-events-none'}`}>
                                        <div className="py-2">
                                            {[
                                                ...
                                                { to: '/settings', icon: ['fas', 'gear'], label: 'Cài đặt' },
                                            ].map(({ to, icon, label }) => (
                                                <Link key={to} to={to} onClick={() => setDropdownOpen(false)} className="flex items-center gap-3 px-5 py-3.5 text-[11px] tracking-[0.15em] uppercase font-bold text-stone-500 hover:text-black hover:bg-stone-50 transition-colors">
                                                    <FontAwesomeIcon icon={icon} className="w-4 text-stone-300" /> {label}
                                                </Link>
                                            ))}
                                            <div className="h-px bg-gray-100 mx-5 my-1" />
                                            <button onClick={handleLogout} className="flex items-center gap-3 w-full text-left px-5 py-3.5 text-[11px] tracking-[0.15em] uppercase font-bold text-red-400 hover:text-red-600 hover:bg-red-50 transition-colors">
                                ...
                                <Link to="/login" className="flex items-center gap-2 text-[12px] tracking-[0.2em] font-bold text-black hover:text-stone-500 transition-colors pl-2">
                                    ĐĂNG NHẬP <FontAwesomeIcon icon={['far', 'user']} className="text-[14px]" />
                                </Link>
                            )}
                        </div>
            ...
            {/* ════════════════════════════════════════
                MOBILE DRAWER (Chỉnh lại Login + Layout)
            ════════════════════════════════════════ */}
            ...
            <div className={`lg:hidden fixed inset-y-0 right-0 w-[280px] bg-white z-50 flex flex-col transition-transform duration-300 ease-in-out shadow-2xl ${menuOpen ? 'translate-x-0' : 'translate-x-full'}`} style={FONT_MONO}>
                ...
                <div className="overflow-y-auto flex-1 py-4">
                    ...
                    <div className="px-6 pb-6 mb-2 border-b border-gray-100">
                        {user ? (
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <p className="text-[12px] font-bold tracking-[0.15em] uppercase text-black">{user.name || user.username}</p>
                                    <button onClick={handleLogout} className="text-[10px] text-stone-400 hover:text-red-500 mt-1 uppercase tracking-wider">Đăng xuất</button>
                                </div>
                            </div>
                        ) : (
                    ...
                    <div className="px-6 space-y-1">
                        {NAV_LINKS.map(({ to, label }) => (
                            <Link key={to} to={to} onClick={() => setMenuOpen(false)} className={`block py-3 text-[12px] tracking-[0.2em] uppercase font-bold transition-colors ${isActive(to) ? 'text-black' : 'text-stone-400 hover:text-black'}`}>
                                {label}
                            </Link>
                        ))}
                        <details className="group">
                            <summary className="flex items-center justify-between py-3 text-[12px] tracking-[0.2em] uppercase font-bold text-stone-400 hover:text-black cursor-pointer list-none transition-colors">
                                Thể loại <FontAwesomeIcon icon={['fas', 'chevron-down']} className="text-[9px] transition-transform duration-200 group-open:rotate-180" />
                            </summary>
                            <div className="pl-4 py-1">
                                {GENRES.map(genre => (
                                    <button key={genre} onClick={() => goToGenre(genre)} className="block w-full text-left py-2.5 text-[11px] tracking-[0.15em] uppercase font-bold text-stone-400 hover:text-black transition-colors">
                                        {genre}
                                    </button>
                                ))}
                            </div>
                    ...
                    {user && (
                        <div className="mt-6 px-6 pt-6 border-t border-gray-100 space-y-1">
                            <p className="text-[9px] uppercase tracking-[0.3em] text-stone-300 font-bold mb-3">Tài khoản</p>
                            {[
                                { to: '/profile', label: 'Hồ sơ cá nhân' },
                                { to: '/orders', label: 'Lịch sử mua hàng' },
                                { to: '/settings', label: 'Cài đặt' },
                            ].map(({ to, label }) => (
                                <Link key={to} to={to} onClick={() => setMenuOpen(false)} className="block py-2.5 text-[11px] tracking-[0.15em] uppercase font-bold text-stone-400 hover:text-black transition-colors">
                                    {label}
                                </Link>
                            ))}
                        </div>
